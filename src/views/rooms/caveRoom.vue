<template>
  <div class="row row-cave position-relative mt-lg-5 mt-3">
        ...
        <template v-if="!endEscapeRoom">
          ...
          <div class="wrap-code-inputs f-center">
            <input class="form-control text-center" @input="checkIfCorrectOrder" v-for="code in codesCorrectOrder"
              v-model="code.input" :key="code.correct" type="number">
          </div>
        </template>
        <template v-else>
    ...
    <div class="col-lg-5 f-center order-lg-2 order-1">
      ...
      <div ref="scroll" class="w-100 f-center">
        <template v-if="!endEscapeRoom">
          <div class="w-75 wrap-scroll position-relative" :class="classFlip" @click="flip" @animationend="flip">
            <template v-if="frontScroll">
            ...
            <template v-else>
              ...
              <div class="wrap-text">
                <h4 class="">הרמב"ם משלב באיגרת פסוקים מן התנ"ך ומסביר אותם. מיצאו באיגרת פסוק ובו מילים כמספר החדרים
                  שפיצחתם
                  עד כה.
                  לכל מילה בפסוק יש "גימטריה קטנה" (ערך מספרי בסיסי ללא אפס. לדוגמא ז=7 וגם ע=7 (במקום 70)). אם תחשבו
                  נכון
        ...
        <template v-else>
          <div class="w-75 wrap-scroll position-relative">
            <img class="w-100" src="@/assets/ass20.png" alt="">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      flip() {
        if (!this.classFlip) {
          this.classFlip = 'to-right'
        } else if (this.classFlip === 'to-right') {
          this.classFlip = 'to-left'
          this.frontScroll = !this.frontScroll
        } else if (this.classFlip === 'to-left') {
          this.classFlip = ''
        }
